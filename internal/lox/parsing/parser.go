...
type SymbolMap struct {
	currentTable    map[string]lox_sc.Token
	previous        *SymbolMap
	currScope       int
	definitions     []lox_sc.Token
...
type Parser struct {
	tokenList       []lox_sc.Token
	errorList       []lox_dg.CompileError
	currentToken    int
	symbolMap       *SymbolMap
	identifierNodes []Node
	references      map[lox_sc.Token][]lox_sc.Token
	scopeTable      map[ScopeRange][]lox_sc.Token
	scopeRanges     []ScopeRange
	panicMode       bool
}

func (parser *Parser) initialize(input []lox_sc.Token) {
	parser.tokenList = input
	parser.currentToken = 0
	parser.errorList = make([]lox_dg.CompileError, 0)
	parser.symbolMap = &SymbolMap{
		currentTable:    make(map[string]lox_sc.Token),
		previous:        nil,
		currScope:       0,
		functionContext: GLOBAL_CONTEXT,
	}
	parser.references = make(map[lox_sc.Token][]lox_sc.Token)
	parser.scopeTable = map[ScopeRange][]lox_sc.Token{}
}
...
func (parser *Parser) Parse(input []Token) ([]Node, []Node, map[Token][]Token, map[ScopeRange][]Token, []lox_dg.CompileError) {
...
func (parser *Parser) addError(message string, source int) {
	...
	parser.errorList = append(parser.errorList, lox_dg.CompileError{Message: message, Line: parser.peekParser().Line, Char: parser.peekParser().Character, Severity: 1, Source: source})
...
func (parser *Parser) addWarning(message string) {
	...
	parser.errorList = append(parser.errorList, lox_dg.CompileError{Message: message, Line: parser.peekParser().Line, Char: parser.peekParser().Character, Severity: 2, Source: ERROR_WARNING})
}
...
func (parser *Parser) addWarningAt(message string, line int, char int) {
	...
	parser.errorList = append(parser.errorList, lox_dg.CompileError{Message: message, Line: line, Char: char, Severity: 2, Source: ERROR_WARNING})
}
...
func (parser *Parser) addErrorAt(message string, line int, char int, source int) {
	...
	parser.errorList = append(parser.errorList, lox_dg.CompileError{Message: message, Line: line, Char: char, Severity: 1, Source: source})
	parser.panicMode = true
}
